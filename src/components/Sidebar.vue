...
<template>
  <aside class="w-64 bg-white border-r border-gray-200 fixed left-0 top-20 bottom-0 z-40 flex flex-col">
    <nav class="px-4 py-6 space-y-2.5 flex-1">
      <router-link
        v-for="item in menuItems"
        :key="item.path"
        :to="item.path"
        class="flex items-center gap-3.5 px-4 py-3.5 rounded-lg text-[15px] no-underline transition-colors"
        :class="
          isActive(item.path)
    ...
    <div v-if="loggedIn" class="border-t border-gray-200 p-4">
      <button
        @click="goToMyPage"
        class="w-full flex items-center gap-3 px-4 py-3.5 rounded-lg hover:bg-gray-100 transition-colors bg-transparent border-none cursor-pointer"
      >
        ...
        <div class="flex-1 text-left">
          <div class="text-[15px] font-medium text-gray-900">{{ userName }}</div>
          <div class="text-[13px] text-gray-500">{{ farmLocation || '지역 미설정' }}</div>
        </div>
      </button>
    </div>
  </aside>
